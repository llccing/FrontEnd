...
define(['jquery',
    ...
    function ($, echarts, common) {
        ...
        model.init = () => {
            // 初始化机构类型
            common.ajaxGet(URL.GET_DEPT_TYPE).done(data=> {
            ...
            // 初始化司法局
            common.ajaxGet(URL.GET_SFJ).done(data => {
                let options = '',
                    arr = data.data;
            ...
            $('#w_district').on('change', () => {
                model.getZdzxWork($('#w_district').val());
            });
            $('#c_district').on('change', () => {
                model.getZdzxReport($('#c_district').val());
            });
            $('#c_zdzx').on('change', () => {
        ...
        model.changeChildPageWidth = () => {
            let width = window.innerWidth - $('.sidebar').width() - 7;
        ...
        model.getZdzx = bm => {
            ...
            common.ajaxGet(URL.GET_FWZDZX, {bm: bm}).done(data => {
        ...
        model.reportSearch = (_data, type) => {
            switch (type) {
                case '1': // 法治宣传
                    common.ajaxGet(URL.TOTAL, _data).done(data => {
                        let arr = data.data;
                        if (arr.length > 0) {
        ...
        // 统计报表 网格统计 区县
        model.areaTable = () => {
            common.ajaxGet(URL.GRID_TOP).done(data => {
                let area = data.data,
                    length = area.length,
                    trs = '';
        ...
        model.streetTable = that => {
            ...
            common.ajaxGet(URL.GRID_CHILDREN, {bm: bmt}).done(data => {
        ...
        model.communityTable = that => {
            let json = $(that).children('.tdJson').html(),
                ...
                y = data.y,
                bm = data.bm;
            common.ajaxGet(URL.GRID_CHILDREN, {bm: bm}).done(data => {
        ...
        model.initTableRmtj = arr => {
            ...
            let trs = '';
            for (let i = 0; i < district.length; i++) {
                trs += '<tr><td>' + district[i] + '</td>' + // 区域
                '<td>' + nums[0][i] + '</td>' + // 总数
                '<td>' + nums[1][i] + '</td>' + // 未处理
                '<td>' + nums[2][i] + '</td>' + // 已处理
                '<td>' + nums[3][i] + '</td></tr>'; // 处理率
            }
